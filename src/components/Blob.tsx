...
function setCSSVars() {
  if (typeof window !== "undefined") {
    ...
    root.style.setProperty("--primary", "rgb(var(--primary) / 0.55)");
    root.style.setProperty("--secondary", "rgb(var(--secondary) / 0.35)");
  }
}

export function Blob({ className }: BlobProps) {
  // ensure CSS vars are set on client
  if (typeof window !== "undefined") setCSSVars();
